...
#ifdef USE_LIBMOBILE
	// Honour an adapter that was switched on before this game was picked.
	if (runner->mobileEnabled) {
		mGUIMobileAdapterAttach(runner);
	}
#endif
...
#ifdef USE_LIBMOBILE
			// Whatever the adapter did during that frame -- a config change to be
			// written out, for one -- is noticed here, once per frame.
